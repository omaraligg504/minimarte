...
function renderCartPage() {
  ...
  container.innerHTML = `
    <div class="category-section">
          ...
          <div class="product-card" data-id="${p.id}">
            ...
            <div class="card-actions">
              <div class="quantity-controls">
                ...
                <span class="qty-value" data-id="${p.id}">${ci.quantity || 1}</span>
      ...
      <div style="text-align:center; margin-bottom:2em;">
        <button id="proceed-checkout" class="checkout-button" ${filtered.length === 0 ? 'disabled' : ''}>Proceed to Checkout</button>
      </div>
    </div>
  `;
  ...
  if (proceedBtn) {
    proceedBtn.addEventListener('click', () => {
      // Save the current cart normalized structure to a checkout-specific key
      // so the checkout page can load the exact items/quantities intended.
      const rawCart = getStorage('cart') || [];
      let cartForCheckout = [];
      if (rawCart.length && typeof rawCart[0] === 'object' && rawCart[0].productId !== undefined) {
        cartForCheckout = rawCart.map(it => ({ productId: it.productId, quantity: it.quantity || 1 }));
      } else {
...
function changeQuantity(productId, delta) {
  // load normalized cart (array of {productId,quantity})
  ...
  // update checkout_cart too if present
  localStorage.setItem('checkout_cart', JSON.stringify(cart));
  // re-render page to update UI
  renderCartPage();
}
